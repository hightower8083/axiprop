...
def get_plasma_J_ADK_OFI(
    ...
    for ir in range(Nr):
        ...
        P_loc_env = np.abs( e * A_ir_t )
        P_loc_re = e * A_ir_t_re
        v_loc_re = P_loc_re / m_e / \
            np.sqrt( 1 + 0.5 * P_loc_env**2 / (m_e*c)**2 )

        if Z_init>0:
            J_ir_t_re += -e * Z_init * n_gas_loc * v_loc_re
        ...
        for it in range(Nt):
            ...
            W_loc = m_e * c**2 * (
                np.sqrt( 1 + 0.5 * P0**2 / (m_e*c)**2 ) - 1.0
            )

            P_follow_re = P_loc_re[it:] + P0
            P_follow_env = P_loc_env[it:]
            v_follow = P_follow_re / m_e / \
                np.sqrt( 1 + 0.5 * P_follow_env**2 / (m_e*c)**2 )
            ...
            for ion_state in range(num_ions-1):
                ...
                if new_events>0:
                    ...
                    all_new_events += new_events
                    electron_temp_dens += W_loc * new_events
                    ...
                    J_ir_t_re[it:] += -e * new_events * n_gas_loc * v_follow
        ...
        J_ir_t = 2 * (phase_env_inv * J_ir_t_re)[::refine_ord]
